// Pure report model shared by the on-screen ExecutiveReport and the JPG/PDF
// exporters (which Share also uses). Both the Entry form (just
...
//
// Materials appear as a "Consumption Entries" annex; only Trust-owned
// entries count toward its totals (lib/materials/consumption.js).

import { normalizeEntry, ownershipCounts, trustTotals } from '../materials/consumption';
...
// Site conditions in display order, with the chip/badge colour class
// (.cond-* in globals.css).
export const CONDITIONS = [
  { value: 'Sunny', cls: 'cond-sunny' },
  { value: 'Cloudy', cls: 'cond-cloudy' },
  { value: 'Rainy', cls: 'cond-rainy' },
  { value: 'Site Closed', cls: 'cond-closed' },
  { value: 'Holiday', cls: 'cond-holiday' },
];
export const conditionClass = (v) => (CONDITIONS.find(c => c.value === v) || {}).cls || '';
 ...
 * @param {Array}  [input.materials] consumption entries (form rows or API logs)
 */
export function buildReport({ date, site, activities, preparedBy = '', editedBy = '', condition = '', projects = [], materials = [], submittedAt = '', draft = false }) {
  ...
  const groups = [...groupMap.values()].map(g => ({
    ...g,
    // Shown on the main-activity row itself, above its sub-activities.
    totals: sumRows(g.rows),
    // A main activity logged on its own (no sub-activities) reads as one row;
    // a totals row above a single identical row would only repeat it.
  ...
  return {
    ...
    consumption: buildConsumption(materials),
...
// submittedAt arrives as an ISO string or epoch millis (older records).
function toIso(v) {
  if (!v) return '';
  const d = new Date(Number.isFinite(Number(v)) ? Number(v) : v);
...
function buildConsumption(materials) {
  const entries = (materials || []).map(normalizeEntry).filter(e => e.material && e.qty > 0);
  return { entries, trustTotals: trustTotals(entries), counts: ownershipCounts(entries) };
}

// Consumption entries logged for the same site + day as a DPR record.
export function consumptionForRecord(item, materialLogs) {
  const ymd = toYMD(item.date);
  const site = String(item.site || '').trim();
  return (materialLogs || []).filter(l => toYMD(l.date) === ymd && String(l.site || '').trim() === site);
}

export function reportFromRecord(item, projects, materialLogs = []) {
  return buildReport({
    date: item.date,
    site: item.site,
    ...
    projects,
    materials: consumptionForRecord(item, materialLogs),
