...
// as real vector text/tables, the WhatsApp text is composed from data, and
// only the JPG rasterises the rendered ExecutiveReport component.

import { createElement } from 'react';
import { flushSync } from 'react-dom';
import { createRoot } from 'react-dom/client';
import ExecutiveReport from '../../components/report/ExecutiveReport';
import { buildWhatsAppText, ORG_NAME, REPORT_TITLE } from './reportModel';
...
function tableBody(report) {
  ...
  report.groups.forEach((g) => {
    ...
    body.push([{ content: pdfText(g.name), colSpan: 6, styles: { fillColor: C.tint, textColor: C.primary, fontStyle: 'bold', halign: 'left' } }]);
    g.rows.forEach((r) => {
      seq += 1;
      body.push([
        seq,
        { content: pdfText(r.name), styles: { cellPadding: { ...pad, left: r.isSub ? 6 : 2.2 } } },
        r.skilled, r.unskilled,
        { content: r.total, styles: { fontStyle: 'bold' } },
        pdfText(r.note),
      ]);
    });
    const sub = { fillColor: C.subtotal, fontStyle: 'bold', textColor: C.text };
    body.push([
      { content: '', styles: sub },
      { content: pdfText(`Subtotal - ${g.name}`), styles: sub },
      { content: g.subtotal.skilled, styles: sub },
      { content: g.subtotal.unskilled, styles: sub },
      { content: g.subtotal.total, styles: sub },
      { content: '', styles: sub },
    ]);
  });
  if (!body.length) {
...
export function shareWhatsApp(report) {
  const url = `https://wa.me/?text=${encodeURIComponent(buildWhatsAppText(report))}`;
  const win = window.open(url, '_blank');
  if (win) win.opener = null;
  else window.location.href = url; // popup blocked (common in installed PWAs) — navigate instead
}

// ── Native share sheet ───────────────────────────────────────────────

// Returns 'shared' | 'shared-text' | 'downloaded' | 'cancelled'.
export async function shareNative(report) {
  const title = `${report.dprNo} — ${report.displayDate}`;
  const text = `${REPORT_TITLE} · ${report.siteDisplay} · ${report.displayDate}`;
  if (typeof navigator === 'undefined' || !navigator.share) {
    await exportPdf(report);
    return 'downloaded';
  }
  const libs = await preloadExportLibs();
  const file = new File([buildPdf(report, libs).output('blob')], `${report.dprNo}.pdf`, { type: 'application/pdf' });
  try {
    if (navigator.canShare && navigator.canShare({ files: [file] })) {
      await navigator.share({ files: [file], title, text });
      return 'shared';
    }
    await navigator.share({ title, text: buildWhatsAppText(report) });
    return 'shared-text';
  } catch (err) {
    if (err && err.name === 'AbortError') return 'cancelled';
    // NotAllowedError (gesture expired) / unsupported payload — still give them the file.
...
export const REPORT_ACTIONS = [
  ...
  { kind: 'whatsapp', label: 'WhatsApp', icon: '💬' },
...
export async function runReportAction(kind, report, toast = () => {}) {
  try {
    if (kind === 'jpg') {
      ...
      if (result === 'downloaded') toast('📄 Sharing isn\'t available here — PDF downloaded instead');
      else if (result !== 'cancelled') toast('📤 Shared');
    }
  } catch (err) {
    console.error(`Report action "${kind}" failed:`, err);
    const label = (REPORT_ACTIONS.find(a => a.kind === kind) || {}).label || kind;
