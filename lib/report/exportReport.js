...
function buildMaterialPdf(report, { jsPDF, autoTable }) {
  ...
  drawMeta(doc, [
    ...
    ['Site', report.siteDisplay],
    ['Logged by', by],
    ['Materials', `${t.materials} material${t.materials === 1 ? '' : 's'}`],
  ]);
  const cur = cursor(doc, drawKpis(doc, [['Consumption entries', t.entries], ['Trust', t.trust], ['Contractor / other', t.reference], ['Materials', t.materials]]) + 9);

  if (report.entries.length) {
    drawConsumption(doc, autoTable, cur, report, { title: 'Consumption entries', withEntriesCol: true });
  } else {
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(9.5);
    doc.setTextColor(...C.muted);
    doc.text('No consumption entries for this site and date.', MARGIN, cur.y + 4);
    cur.y += 10;
  }

  drawClosing(doc, cur, `Logged by - ${by}`,
    `${report.dprNo}  |  Last logged: ${formatDateTime(report.submittedAt) || '-'}  |  Generated ${formatDateTime(report.generatedAt)}`);
  return doc;
}
