...
// One-line caption sent with the file: `DPR_<Site>_<DD-MM-YYYY>` or
// `MCR_<Site>_<DD-MM-YYYY>` (e.g. DPR_Dahod_Trimandir_24-09-2026).
export function shareCaption(report) {
  const prefix = report.kind === 'material' ? 'MCR' : 'DPR';
  const site = String(report.site || '').trim().replace(/[^A-Za-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || 'Site';
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(report.date || '');
  return `${prefix}_${site}_${m ? `${m[3]}-${m[2]}-${m[1]}` : 'undated'}`;
}
