...
export async function saveMaterialLog(body, actor) {
  ...
  const { data: saved, error } = await supabase.from('material_logs').insert(rows).select('id, material_name, created_at');
  if (error) return { error: consumptionColumnsMissing(error) ? MIGRATION_HINT : error.message };
  return { status: 'ok', inserted: rows.length, entries: (saved || []).map(r => ({ id: r.id, material_name: r.material_name, createdAt: r.created_at })) };
}
